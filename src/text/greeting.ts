...
const greeting = () => async (ctx: Context) => {
  ...
  if (!messageId || !chatId) {
    debug('Invalid context: missing messageId or chatId');
    return;
  }

  try {
    debug(`Saving chat ID: ${chatId}`);
    ...
    debug(`Error in greeting: ${errorMessage}`);
...
const search = () => async (ctx: Context) => {
  ...
  try {
    debug(`Searching for partner for chat ID: ${chatId}`);
    await axios.post(GOOGLE_SHEET_URL, { action: 'saveChatId', chatId });
    ...
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    debug(`Error in search: ${errorMessage}`);
...
const stop = () => async (ctx: Context) => {
  ...
  try {
    debug(`Stopping chat for chat ID: ${chatId}`);
    ...
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    debug(`Error in stop: ${errorMessage}`);
...
const link = () => async (ctx: Context) => {
  ...
  try {
    debug(`Requesting link for chat ID: ${chatId}`);
    ...
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    debug(`Error in link: ${errorMessage}`);
...
const share = () => async (ctx: Context) => {
  ...
  try {
    debug(`Sharing profile for chat ID: ${chatId}`);
    const response = await axios.post(GOOGLE_SHEET_URL, { action: 'getPartner', chatId });
    const { partnerId, isLive } = response.data;
    ...
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    debug(`Error in share: ${errorMessage}`);
